...
def setup_base_data(conn):
    ...
    # --- CORREÇÃO DO BUG (Baseado na sua ideia) ---
    # Insere a marca principal (BRAND_ID = 1) ANTES de tentar usá-la.
    # ON CONFLICT (id) DO NOTHING torna seguro executar isto múltiplas vezes.
    try:
        cursor.execute(
            "INSERT INTO brands (id, name, created_at) VALUES (%s, %s, %s) ON CONFLICT (id) DO NOTHING",
            (BRAND_ID, 'Nola Restaurants', datetime.now())
        )
        print(f"✓ Marca principal (ID: {BRAND_ID}) assegurada.")
    except Exception as e:
        print(f"Erro ao inserir marca principal: {e}")
        conn.rollback()
        raise
    # --- FIM DA CORREÇÃO ---
